...
paste("The largest win spread is", max(spread), "when the Bears scored", bears[which.max(spread)], "points and the opponent scored", opponent[which.max(spread)])
```
...
```{r}
game_score <- rbind(game_score, matrix(c(37,9,17,17),2,2))
game_score
```
...
for (i in 1:nrow(game_score)) {
  spread_mat <- game_score[i, 1] - game_score[i, 2]
  if (spread_mat > 0) {
    print("Win")
  } else {
    print("Loss")
...
```{r}
area_of_rect <- function(length = 48, width = 48) {
  area_in = length*width
  area_ft = area_in/144
  return (c(area_in, area_ft))
}
areas <- area_of_rect(length = 102, width = 60)
paste("The area in square inches is", areas[1])
paste("The area in square feet is", areas[2])
```
...
most_letters <- function(vec) {
  word_lengths <- sapply(vec, function(x) length(strsplit(x, '')[[1]]))
  return(vec[word_lengths == max(word_lengths)])
}
most_letters(fruits)
```
...
```{r}
value_in_vec <- function(vec, val) {
  return(any(vec == val))
}
value_in_vec(check, value)
if (value_in_vec(check, value)) {
